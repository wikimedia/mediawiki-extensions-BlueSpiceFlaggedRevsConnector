...
class FRCDiffView extends ContextSource {
	...
	public function setRobotPolicy() {
		...
		if ($this->article->getStableRev() && $this->article->isStableShownByDefault()) {
			...
			} elseif (!$request->getVal('stableid') && $this->out->getRevisionId() == $this->article->getStable() && $this->article->stableVersionIsSynced()) {
	...
	public function imagePageFindFile(&$normalFile, &$displayFile) {
		...
		if ($frev) {
			...
			if (!$time) {
				...
				$time = $dbr->selectField('flaggedimages', 'fi_img_timestamp', array('fi_rev_id' => $frev->getRevId(),
					'fi_name' => $this->article->getTitle()->getDBkey()), __METHOD__
				);
		...
		if ($srev && $this->article->revsArePending()) {
			...
			if ($revsSince) {
				$s .= "<div class='flaggedrevs_editnotice plainlinks'>" .
						$this->msg('revreview-pending-nosection', $srev->getRevId(), $revsSince)->parse() . "</div>";
			}
		}
		return true;
	}
	...
	public function addToCategoryView() {
		...
		if (!FlaggedRevs::useOnlyIfProtected()) {
			...
			$this->out->addSubtitle(
					Html::rawElement(
							'span', array('class' => 'plainlinks', 'id' => 'mw-fr-category-oldreviewed'), $this->msg('flaggedrevs-categoryview', urlencode($category))->parse()
					)
			);
		}
		return true;
	...
	public function addReviewForm(&$output) {
		...
		if ($rev && (!$this->diffRevs || $this->isReviewableDiff )) {
			...
			if ($this->out->getRevisionId() == $rev->getId() && empty($this->out->fr_unversionedIncludes)) {
				...
				list( $tmpVers, $fileVers ) = FRInclusionCache::getRevIncludes($this->article, $rev, $reqUser);
			}
		...
		if ($action == 'protect' || $action == 'unprotect') {
			...
			if ($frev && $frev->getRevId() == $this->article->getLatest()) {
				$this->out->prependHTML("<span class='plainlinks'>" .
						$this->msg('revreview-visibility-synced', $title->getPrefixedText())->parse() . "</span>");
			} elseif ($frev) {
				$this->out->prependHTML("<span class='plainlinks'>" .
						$this->msg('revreview-visibility-outdated', $title->getPrefixedText())->parse() . "</span>");
			} else {
				$this->out->prependHTML("<span class='plainlinks'>" .
						$this->msg('revreview-visibility-nostable', $title->getPrefixedText())->parse() . "</span>");
			}
		}
		return true;
	}
	...
	public function setActionTabs($skin, array &$actions) {
		...
		$allowedStable = MediaWikiServices::getInstance()->getPermissionManager()->userHasRight(
			$reqUser,
			'stablesettings'
		);
	...
	protected function addDraftTab(array &$views, FlaggedRevision $srev, $type) {
		...
		if ($this->showingStable() || $request->getVal('stableid')) {
			...
			if ($this->out->getRevisionId()) { // @TODO: avoid same query in Skin.php
				$ts = ( $this->out->getRevisionId() == $this->article->getLatest() ) ? $this->article->getTimestamp() // skip query
	...
	public function addToDiffView($diff, $oldRev, $newRev) {
		...
		if ($srev && $this->isDiffFromStable && !$this->article->stableVersionIsSynced()) { // pending changes
			$changeText = '';
	...
	FlaggableWikiPage $article, $oldRev, Revision $newRev
		...
		if ($article->revsArePending() && !$fullStableDiff) {
			$review = Linker::linkKnown(
							$article->getTitle(), wfMessage('review-diff2stable')->escaped(), array(), array('oldid' => $srev->getRevId(), 'diff' => 'cur') + FlaggedRevs::diffOnlyCGI()
			);
	...
	protected static function fetchTemplateChanges(FlaggedRevision $frev, $newTemplates = null) {
		...
		foreach ($changes as $tuple) {
			...
			$link = Linker::linkKnown(
							$title, htmlspecialchars($title->getPrefixedText()), array(), array('diff' => 'cur', 'oldid' => $revIdStable));
			if (!$hasStable) {
	...
	public function injectPostEditURLParams(&$sectionAnchor, &$extraQuery) {
		...
		if ($frev->userCanSetFlags($reqUser)) {
			...
			if ( !$allowedReview && $this->article->isStableShownByDefault() ) {
	...
	protected function editWillRequireReview(EditPage $editPage) {
		...
		if (!$this->editRequiresReview($editPage)) {
			...
			if ( $request->getCheck( 'wpReviewEdit' ) && $canReview ) {
	...
	public function addReviewCheck(EditPage $editPage, array &$checkboxes, &$tabindex) {
		...
		if ( !$this->article->isReviewable() || !$canReview ) {
		...
		if (self::getBaseRevId($editPage, $request) == $this->article->getLatest()) {
			...
			$checkbox = Xml::check(
							'wpReviewEdit', $request->getCheck('wpReviewEdit'), array('tabindex' => ++$tabindex, 'id' => 'wpReviewEdit')
			);
	...
	protected static function getBaseRevId(EditPage $editPage, WebRequest $request) {
		if (!isset($editPage->fr_baseRevId)) {
			...
			if ($undo && $undo === $latestId) {
				...
				$revId = $article->getOldID() ? $article->getOldID() : $request->getInt('baseRevId'); // e.g. "show changes"/"preview"
			}
