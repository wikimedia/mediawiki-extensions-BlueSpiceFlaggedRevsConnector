...
class FRCInfobox {
	 ...
	 */
	public function onBSInfoBoxBeforeEntryViewAddData( &$aData, $args, $oInfoBoxListEntryView, $row = null ) {
		...
		$aConditions = array('rc_cur_id = '.$iPageId);
		$res = $dbr->selectRow(
			array( 'recentchanges' ), array( 'rc_comment' ), $aConditions, __METHOD__, array( 'ORDER BY' => $sOrderSQL, 'LIMIT' => 1 )
		);
		...
		$aFields = array( 'fr_timestamp' );

		$sOrderSQL   = 'fr_timestamp DESC';
		$aConditions = array('fr_page_id = '.$iPageId);
		$res = $dbr->selectRow(
			array( 'flaggedrevs' ), $aFields, $aConditions, __METHOD__, array( 'ORDER BY' => $sOrderSQL, 'LIMIT' => 1 )
		);
		...
		$aData['DATE'] = $sStableDate ? '<b>'.$wgLang->sprintfDate( 'l, d. F Y', $sStableDate ).'</b><br />' : '';

		$oInfoBoxListEntryView->setTemplate( $sUlSuffix.'{DATE}'.$sUlPrefix.'<li>[[:{PREFIXEDTITLE}|{DISPLAYTITLE}]]{TEXT}</li>'.$sULLastPrefix );
		return true;
	}
	...
	public function onBSInfoBoxCustomMode( &$aObjectList, $args ) {
		...
		try {
			...
			$aConditions[] = '(SELECT page_namespace FROM '.$dbr->tableName('page').' WHERE page_id = fr_page_id) IN ('.implode( ',',$aNamespaceIds ).')';
		}
		...
		$aTables = array( 'flaggedrevs' );

		$aFields = array(   '(SELECT page_title FROM ' . $dbr->tableName( 'page' ) . ' WHERE page_id = fr_page_id) as title',
							'(SELECT page_namespace FROM ' . $dbr->tableName( 'page' ) . ' WHERE page_id = fr_page_id) as namespace'
						);
		$aOptions = array(  'GROUP BY' => 'title,namespace',
							'ORDER BY' => $sOrderSQL, 'LIMIT' => $args[ 'count' ]
						);

		$sPendingCondition = (!$args[ 'showpending' ]) || $args[ 'showpending' ] != true ? ' where fp_pending_since IS NULL ' : '';
		$aConditions[ ] = 'fr_page_id IN ( SELECT fp_page_id FROM ' . $dbr->tableName( 'flaggedpages' ) . $sPendingCondition . ')';
