...
class FRCBookshelf {
	 ...
	 * @param $aTemplate array
	 * @param $aBookPage Title
	 * @param $aArticles array
	 * @return bool Always true to keep hook running
	 * @global $wgLang Languange
	 */
	public function onBSBookshelfExportBeforeArticles( &$aTemplate, &$aBookPage, &$aArticles ) {
		...
		if( !$bShowStable && !$bShowNoStable && !$bShowNoFR ) return true; //In this case we do not need to do anything

		//Let's add the "FlaggedRevs History page"
		$oFlaggedRevsHistoryPage = $aTemplate[ 'dom' ]->createElement( 'div' );
		$oFlaggedRevsHistoryPage->setAttribute( 'class', 'bs-section bs-custompage bs-flaggedrevisionhistorypage bs-flaggedrevshistorypage' );
		...
		$aArticleIds    = array();
		$aStables       = array(); //ids
		$aNoStables     = array(); //ids
		$aNoFalggedRevs = array(); //article arrays
		...
		if ( empty( $aArticleIds ) ) {
			...
			$aStableRows = array();
			foreach ( $res as $oRow ) {
				$aStables[] = $oRow->log_page; //This is a stable page
				$aStableRows[] = $oRow;
			}
		}
		...
		if( $bShowStable && !empty( $aStables ) ) {
			...
			$oStableDiv->appendChild( $aTemplate[ 'dom' ]->createElement( 'h2', wfMessage( 'bs-flaggedrevsconnector-flaggedrevshistorypage-stableversionstabletitle' )->plain() ) );

			$oStableVersionsTable = $aTemplate[ 'dom' ]->createElement( 'table' );
			$oStableVersionsTable->setAttribute( 'width', '100%' );
			$oStableVersionsTable->setAttribute( 'class', 'bs-flaggedrevshistorypage-stables-list' );
			$oStableDiv->appendChild( $oStableVersionsTable );

			$oTHead = $oStableVersionsTable->appendChild( $aTemplate[ 'dom' ]->createElement( 'thead' ) );
			$oTBody = $oStableVersionsTable->appendChild( $aTemplate[ 'dom' ]->createElement( 'tbody' ) );
			$oTHRow = $oTHead->appendChild( $aTemplate[ 'dom' ]->createElement( 'tr' ) );
			$oTHRow->appendChild( $aTemplate[ 'dom' ]->createElement( 'th', wfMessage( 'bs-flaggedrevsconnector-flaggedrevshistorypage-stabledate' )->plain() ) );
			$oTHRow->appendChild( $aTemplate[ 'dom' ]->createElement( 'th', wfMessage( 'bs-flaggedrevsconnector-flaggedrevshistorypage-title' )->plain() ) );
			$oTHRow->appendChild( $aTemplate[ 'dom' ]->createElement( 'th', wfMessage( 'bs-flaggedrevsconnector-flaggedrevshistorypage-comment' )->plain() ) );
		...
		if( $bShowNoStable && !empty( $aNoStables ) ) {
			...
			$oNoStableDiv->appendChild( $aTemplate[ 'dom' ]->createElement( 'h2', wfMessage( 'bs-flaggedrevsconnector-flaggedrevshistorypage-nostableversionstabletitle' )->plain() ) );
		...
		if( $bShowNoFR && !empty( $aNoFalggedRevs ) ) {
			...
			$oNoFRDiv->appendChild( $aTemplate[ 'dom' ]->createElement( 'h2', wfMessage( 'bs-flaggedrevsconnector-flaggedrevshistorypage-noflaggedrevstitle' )->plain() ) );
