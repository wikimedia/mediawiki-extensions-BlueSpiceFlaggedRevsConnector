	...
	public function onBSBookshelfExportBeforeArticles( &$aTemplate, &$aBookPage, &$aArticles ) {
		...
		if ( $bShowNoStable && !empty( $aNoStables ) ) {
			...
			$oNoStableDiv->appendChild( $aTemplate[ 'dom' ]->createElement(
				'h2',
				wfMessage( 'bs-flaggedrevsconnector-flaggedrevshistorypage-nostableversionstabletitle' )->plain()
			) );
		...
		if ( $oStableDiv != null ) {
			$oFlaggedRevsHistoryPage->appendChild( $oStableDiv );
		}
		if ( $oNoStableDiv != null ) {
			$oFlaggedRevsHistoryPage->appendChild( $oNoStableDiv );
		}
		if ( $oNoFRDiv != null ) {
			$oFlaggedRevsHistoryPage->appendChild( $oNoFRDiv );
		}
