...
class PrimaryDataProvider extends PageDataProvider {
	...
	private function initReadNamespaces() {
		...
		foreach ( $namespaces as $nsId => $nsName ) {
			...
			$userCanRead = $title->userCan( 'read', $user );
			if( $userCanRead ) {
	 ...
	 * Permission checks are very expensive, so for performance reasons permission is not checked for each specific page,
	 * but for page namespace instead.
	 *
